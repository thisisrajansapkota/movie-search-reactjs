      ...
      {movieList.length > 0 && (
        <>
          ...
          <ButtonGroup aria-label="Basic example">
            ...
            <Button variant="success" onClick={() => filterMovies("awesome")}>
              Awesome
            </Button>
            <Button variant="warning" onClick={() => filterMovies("boring")}>
              Boring
            </Button>
          </ButtonGroup>
          <div className="d-flex flex-wrap gap-3 justify-content-between">
            {filteredList.map((movie) => {
              return (
                <MovieCard movie={movie} removeMovie={removeFromMovieList} />
              );
            })}
          </div>
        </>
